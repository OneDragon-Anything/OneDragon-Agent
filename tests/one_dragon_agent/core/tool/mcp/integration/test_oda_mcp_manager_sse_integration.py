"""Integration tests for OdaMcpManager with SSE MCP server.

Tests start SSE MCP server processes, use McpToolSet to connect to them,
...
class TestOdaMcpManagerSSEIntegration:
    """Test OdaMcpManager with SSE MCP server integration.
    
    This test class starts SSE MCP server processes, uses McpToolSet to connect
    ...
    async def test_setup(self, required_env_vars):
        """Create complete test setup with SSE MCP server and OdaAgent.
        ...
        # Create SSE MCP configuration (MCPToolSet will connect to server)
        app_name = "test_sse_app"
        ...
        await mcp_manager.register_custom_config(sse_config)
    ...
    async def test_sse_mcp_toolset_creation(self, test_setup):
        """Test McpToolSet creation with SSE MCP server.
        
        This test validates that OdaMcpManager can successfully create
        an MCPToolset that connects to a real SSE MCP server.
        """
        mcp_manager = test_setup["mcp_manager"]
        app_name = test_setup["app_name"]
        mcp_id = test_setup["mcp_id"]
        
        # Create MCPToolset (this will connect to SSE server)
        toolset = await mcp_manager.create_mcp_toolset(app_name, mcp_id)
        
        # Validate toolset was created
        assert toolset is not None, "SSE MCPToolset should be created"
        ...
        print("Successfully created SSE MCPToolset with real SSE MCP server")
    
    @pytest.mark.asyncio
    @pytest.mark.timeout(120)
    async def test_oda_agent_with_sse_mcp_tools(self, test_setup):
        """Test OdaAgent integration with SSE MCP tools from real server.
        
        This test validates that OdaAgent can successfully use tools
        from a real SSE MCP server through McpToolSet integration.
        """
        mcp_manager = test_setup["mcp_manager"]
        app_name = test_setup["app_name"]
        mcp_id = test_setup["mcp_id"]
        ...
        # Create LlmAgent with SSE MCP tools
        ...
        # Create LlmAgent with SSE MCP toolset
        agent_config = create_default_agent_config(app_name)
        llm_agent_with_tools = LlmAgent(
            name=f"{agent_config.agent_name}_with_sse_tools",
            model=glm_model,
        ...
        # Test: Use SSE MCP weather tool through OdaAgent
        event_count = 0
        ...
        async for event in oda_agent_with_tools.run_async(test_message):
            ...
            if event.is_final_response():
                ...
                if event.content and event.content.parts:
                    ...
                    print(f"SSE MCP tool response: {response_content}")
        ...
        print("Successfully tested OdaAgent with SSE MCP tools from real server")
    
    @pytest.mark.asyncio
    @pytest.mark.timeout(120)
    async def test_sse_mcp_tool_listing_and_accessibility(self, test_setup):
        """Test that SSE MCP tools are properly listed and accessible.
        
        This test validates that SSE MCP server's tools are properly
        ...
        print("Successfully validated SSE MCP tool listing and accessibility")
    
    @pytest.mark.asyncio
    @pytest.mark.timeout(120)
    async def test_sse_mcp_server_reconnection(self, test_setup):
        """Test SSE MCP server reconnection and stability.
        
        This test validates that SSE MCP connection remains stable
        ...
        # Create session
        user_id = "test_sse_reconnect_user"
        session_id = "test_sse_reconnect_session"
        await session_service.create_session(app_name=app_name, user_id=user_id, session_id=session_id)
        ...
        print("Successfully tested SSE MCP server reconnection and stability")